...
const Vector3D Camera::get_ray_direction(const double i, const double j) const
{
	Vector3D dir(0.0, 0.0, 0.0);
	dir += -w * focalDistance;
	//For random supersampling
	// float xw = aspect*(i - width/2.0 + 0.5)/width;
	// float yw = (j - height/2.0 + 0.5)/height;
	float xw = aspect*(i - width/2.0 )/width;
	float yw = (j - height/2.0 )/height;
	dir += u * xw;
	dir += v * yw;
